...
func enter() -> void:
	...
	enemy.disengage_area.connect("body_exited", _on_disengage_area_body_exited)
	
	target = enemy.disengage_area.get_overlapping_bodies()[0]
...
func exit() -> void:
	...
	enemy.disengage_area.disconnect("body_exited", _on_disengage_area_body_exited)
